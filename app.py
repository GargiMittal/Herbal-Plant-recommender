    ...
    with open('herbal_plants_soil.csv', newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            soil_plant_map[row['Soil Type']] = [
                row['Herbal Plant 1'], row['Herbal Plant 2'], row['Herbal Plant 3'],
                row['Herbal Plant 4'], row['Herbal Plant 5']
            ]
...
def classic_index():
    states = list(state_soil_map.keys())
    soils  = list(herbal_plant_map.keys())
    return render_template('classic/index.html', states=states, soils=soils)
 
@app.route('/classic/recommend', methods=['POST'])
def classic_recommend():
    ...
    if not soil_type:
        soil_type = state_soil_map.get(state)
    plants = herbal_plant_map.get(soil_type, [])
    return render_template('classic/results.html', plants=plants, soil_type=soil_type, state=state)
 
@app.route('/classic/plant/<plant_name>')
def classic_plant_details(plant_name):
    plant_info = plant_info_map.get(plant_name, {})
    return render_template('classic/plant_details.html', plant_name=plant_name, plant_info=plant_info)
